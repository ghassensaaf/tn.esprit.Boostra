...
public class ReclamationService implements IReclamationService {
  ...
  @Autowired ReclamationRepository rr;
  ...
  public Reclamation SetReclamtionTotraiter(Reclamation reclamation) {
    ...
    if ((reclamation.getStatuR().equals("traitée")) &&
        (rec.getTypeReclamation() == TypeRec.technical)) {
      ...
      Message message =
          Message
              .creator(new com.twilio.type.PhoneNumber(
                           "+216" + rec.getUser().getNumber()),
                       new com.twilio.type.PhoneNumber("+16815324886"),
                       "Bonjour " + rec.getUser().getFirstName() + " " +
                           rec.getUser().getLastName() + " "
                           + "Votre Reclmation N°" + rec.getId() + " "
                           + " a été traitée Merci pour votre confiance !")
              .create();
    }
    return rr.save(rec);
  };
  ...
  public boolean profanityDetection(String text) {
    ...
    try {
      ...
      System.err.println(
          "Exception when calling AnalyticsApi#analyticsSentiment");
      e.printStackTrace();
    }
    return false;
  }
  ...
  public void UpdateSystemReclamation() {
    List<Reclamation> reclamations =
        rr.TraiterSystemReclamation(TypeRec.System);
    for (Reclamation reclamation : reclamations) {
      ...
      Message message =
          Message
              .creator(new com.twilio.type.PhoneNumber(
                           "+216" + reclamation.getUser().getNumber()),
                       new com.twilio.type.PhoneNumber("+16815324886"),
                       "Bonjour " + reclamation.getUser().getFirstName() + " " +
                           reclamation.getUser().getLastName() + " "
                           + "Votre Reclmation N°" + reclamation.getId() + " "
                           + "a été traitée Merci pour votre confiance !")
              .create();
    }
