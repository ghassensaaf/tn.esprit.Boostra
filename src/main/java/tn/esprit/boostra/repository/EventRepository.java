...
public interface EventRepository extends CrudRepository<Event, Long> {
  ...
  List<Event>
  findAllBystartDateLessThanEqualAndBystartDateGreaterThanEqual(Date d);
  ...
  @Query(
      "select e from Event e where e.startDate > ?1 and e.endDate < ?2 and e.location like %?3% and e.maxParticipants - e.participantCount >= ?4 ")
  List<Event>
  searchEvents(Date startDate, Date endDate, String location, int nbrPart);
}
