...
public interface MessageRepository extends CrudRepository<Message, Long> {
	...
	List<Message> findBySenderAndReceiver(@Param ("idSender")long idSender, @Param("idReceiver") long idReceiver);
	
 
	@Query("select m from Message m where (m.sender.id=:idSender and m.receiver.id=:idReceiver) or"
			+ " (m.sender.id=:idReceiver and m.receiver.id=:idSender) order by date desc")
	List<Message> findBySenderAndReceiverdesc(@Param ("idSender")long idSender, @Param("idReceiver") long idReceiver);
}